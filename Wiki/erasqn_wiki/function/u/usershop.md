...
将用户上一步的输入记录在`CHOICE`和`TFLAG:102`中
...
如果用户在`@SHOW_SHOP`中输入的是`113`。

当`CHARANUM`角色总数大于`2`时，就调用`@COSPLAY_LIST, 0, 0`。

**L692-L693**：

如果用户在`@SHOW_SHOP`中输入的是`114`。

就调用`@EVENT_SHOP`。

**L695-L700**：

如果用户在`@SHOW_SHOP`中输入的是`115`。

当`NUM("総衣装数")`游戏内所有衣装的总数大于`NUM("所持衣装")`玩家持有的衣装数时，就调用`@COSPLAY_SHOP`，否则打印`已卖完`的消息。

**L702-L703**：

如果用户在`@SHOW_SHOP`中输入的是`116`。

就调用`@ARCANA_SHOP`。

**L704-L705**：

如果用户在`@SHOW_SHOP`中输入的是`117`。

就调用`@ACCESSORY_SHOP`。

**L705-L713**：

如果用户在`@SHOW_SHOP`中输入的是`118`。

当`FLAG:40`(食品代码，推测吃东西后会被更新)为`0`时，就调用`@EAT_LUNCH`，否则打印`已吃不下`。

**L715-L728**：

如果用户在`@SHOW_SHOP`中输入的是`120`。
...
如果用户在`@SHOW_SHOP`中输入的是`121`。

就调用`@EXPLORER, 0, 0`

**L734-L739**：

如果用户在`@SHOW_SHOP`中输入的是`122`。
...
如果用户在`@SHOW_SHOP`中输入的是`200`。

就调用`@SAVEGAME_EX`。

**L812-L813**：

如果用户在`@SHOW_SHOP`中输入的是`300`。

就调用`@LOADGAME_EX`。

**L815-L822**：

如果用户在`@SHOW_SHOP`中输入的是`330`，

且`ITEM:化蝶之梦`的值为`100`时，就打印`化蝶之梦已达尾声`，让用户选择是否醒来。

如果选择醒来，就调用`@EVENT_KOTYOUNOYUME, "夢の終わり"`。

**L823-L824**：

如果用户在`@SHOW_SHOP`中输入的是`333`，

且拥有`ITEM:等级限制器`，就调用`@LV_LIMITTER`。

**L825-L826**：

如果用户在`@SHOW_SHOP`中输入的是`555`，

则调用`@SET_CONFIGURE`，进行系统设定。

**L827-L849**：

如果用户在`@SHOW_SHOP`中输入的是`600`。
...
**L870-L871**：如果用户在`@SHOW_SHOP`中输入的是`777`，

则调用`@NAME_CONVERT`（角色改名功能，与系统设置里面的项目重复，且无直接对应的菜单项目）。

**L872-L881**：

如果用户在`@SHOW_SHOP`中输入的是`888`。
...
如果用户在`@SHOW_SHOP`中输入的是`88224646`。
